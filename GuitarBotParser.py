...
class GuitarBotParser:
    ...
    def parseright_M(right_arm, measure_time):
        ...
        GuitarBotParser.print_Events(rh_events)
        ...
        print("\nRH MM:")
        GuitarBotParser.print_Events(rh_motor_positions)
        ...
        for measure in left_arm:
            ...
            for chords in measure:
                if len(chords) != 0:
                    ...
                    # frets, command, dtraj, utraj = GuitarBotParser._get_chords_M("Chords - Chords.csv", note + key, type)
                    frets, command, dtraj, utraj = GuitarBotParser._get_chords_M("Alternate_Chords.csv", note + key, type)
    ...
    def getFullMatrix(events_list, initial_point, plot=False):
        ...
        for key, value in full_matrix.items():
            ...
            i += 1
        if plot:
            GuitarBotParser.plot_interpolation(mode="matrix", matrix=full_matrix)
        return full_matrix
    ...
    def parseAllMIDI(chords, pluck, initial_point, graph=True):
        ...
        lh_dictionary, pick_dictionary = GuitarBotParser.interpolateEvents(lh_motor_positions, picker_motor_positions_adj,
