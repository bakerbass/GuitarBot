...
def arms_to_MIDI(left_arm, right_arm, tempo):
    # TODO: meaningfully use right_arm, doesn't play in as often as it should...
    MIDI_song = []
    
    curr_chord_name = ""
    curr_chord_subdivisions = 0
    is_downstrum = True

    m = 0   # measure index
    sd = 0  # subdivision index
    
    while m < len(left_arm):
        while sd < len(left_arm[m]):
            if left_arm[m][sd] != "":
                curr_chord_name = left_arm[m][sd]
                curr_chord_subdivisions = 1
                is_downstrum = right_arm[m][sd].lower() == "d"
                sd += 1
                break # found first chord
            sd += 1
        sd = 0
        m += 1
    
    if m == len(left_arm):
        return MIDI_song # empty, no chords

    while m < len(left_arm): # measure index
        while sd < len(left_arm[m]): # subdivision index
            if left_arm[m][sd] != "":
                MIDI_song.append(chord_tab_to_MIDI(curr_chord_name, curr_chord_subdivisions, tempo, is_downstrum))
                curr_chord_name = left_arm[m][sd]
                curr_chord_subdivisions = 1
                print("empty case... carry over from before")
            else:
                curr_chord_subdivisions += 1
...
def chord_tab_to_MIDI(chord, subdivisions, tempo, is_downstrum):
    ...
    for fret_number_of_note in chord:
        MIDI_chord.insert(chord_idx, Message("note_off", note=open_note+fret_number_of_note, time=duration, channel=0))
        MIDI_chord.insert(chord_idx, Message("note_on", note=open_note+fret_number_of_note, velocity=80, channel=0))
        chord_idx += 1
